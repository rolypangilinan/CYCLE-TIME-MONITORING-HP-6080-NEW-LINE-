...
class DatabaseManager:
    ...
    def connect(self):
        ...
        try:
            ...
            # Drop old columns if they exist and recreate table with new structure
            try:
                cursor.execute("ALTER TABLE kitting_summary DROP COLUMN IF EXISTS kitting_no")
                cursor.execute("ALTER TABLE kitting_summary DROP COLUMN IF EXISTS material_description")
            ...
            try:
                cursor.execute("ALTER TABLE kitting_summary ADD COLUMN IF NOT EXISTS mtrl_desc VARCHAR(500) AFTER row_no")
                cursor.execute("ALTER TABLE kitting_summary ADD COLUMN IF NOT EXISTS scan_mtrl VARCHAR(100) AFTER qty_unit")
            except:
                pass
            ...
            material_tables = [
                ...
                (14, 'tbl_ground_wire'),
                (15, 'tbl_vcr'),
                (16, 'tbl_lower_housing'),
                (17, 'tbl_rubber_foot'),
                (18, 'tbl_power_cord'),
                (19, 'tbl_partition_gasket'),
                (20, 'tbl_upper_housing'),
                (21, 'tbl_filter_cover'),
                (22, 'tbl_filter'),
                (23, 'tbl_sound_absorbing'),
                (24, 'tbl_manual'),
                (25, 'tbl_l_hose_set'),
                (26, 'tbl_filter_bush')
            ]
    ...
    def save_kitting_summary(self, job_order, model_code, summary_rows):
        ...
        Args:
            job_order: Job order number
            model_code: Model code
            summary_rows: List of dicts with row_no, mtrl_desc, qty_unit, scan_mtrl, lot_no, qty_kit
        """
        connection = None
        cursor = None
        try:
            ...
            (job_order, model_code, row_no, mtrl_desc, qty_unit, scan_mtrl, lot_no, qty_kit, date_today)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
            ON DUPLICATE KEY UPDATE
                model_code = VALUES(model_code),
                mtrl_desc = VALUES(mtrl_desc),
                qty_unit = VALUES(qty_unit),
                scan_mtrl = VALUES(scan_mtrl),
                lot_no = VALUES(lot_no),
    ...
    def get_kitting_summary(self, job_order):
        ...
        try:
            ...
            query = """
            SELECT row_no, mtrl_desc, qty_unit, scan_mtrl, lot_no, qty_kit 
    ...
    def save_to_material_tables(self, kitting_no, job_order, model_code, materials, kitting_qr_code):
        ...
        try:
            ...
            for material in materials:
                ...
                if not table_name:
                    print(f"No table mapping for row_no {row_no}")
                    continue
                ...
                values = (
                    ...
                    model_code,
                    material.get('mtrl_desc', ''),
                    int(material.get('qty_unit', 0) or 0),
                    material.get('scan_mtrl', ''),
                    material.get('lot_no', ''),
                    int(material.get('qty_kit', 0) or 0),
                    kitting_qr_code,
                    today
                )
            ...
            connection.commit()
            print(f"Saved {len(materials)} materials to their respective tables for kitting #{kitting_no}")
            return True
