...
def material_setter():
    # Show the material setter page
    return render_template('material_setter.html')
    ...
    try:
        ...
        if df is not None and len(df) > 0:
            ...
            for col in columns:
                ...
                # Find Material code column (exact match for "Material" or "matl")
                if col_lower == 'material' or col_lower == 'matl' or col_lower == 'materialcode':
                    material_code_col = col
            
            # If Material column not found, try partial match
            if material_code_col is None:
                for col in columns:
                    col_lower = col.lower()
                    # Match "matl" but not "materialdescription"
                    if 'matl' in col_lower and 'description' not in col_lower:
                        material_code_col = col
                        break
            
            print(f"MaterialDescription column: {material_desc_col}, Material Code column: {material_code_col}")
                ...
                for idx, row in df.iterrows():
                    ...
                    if matches:
                        ...
                        if row_no not in matched_materials:
                            ...
                            print(f"Row {row_no} ({category}): {mat_desc} | Code: {mat_code}")
            else:
...
def load_model_codes_file():
    ...
    return ['60CAT0213P', '60CAT0212P']  # Default models without bushing
