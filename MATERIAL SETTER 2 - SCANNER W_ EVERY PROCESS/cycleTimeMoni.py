        ...
        if df is not None and len(df) > 0:
            ...
            if material_desc_col:
                ...
                for idx, row in df.iterrows():
                    ...
                    # Get matl_qty (10th column) for this row
                    matl_qty = ''
                    if matl_qty_col:
                        matl_qty = str(row[matl_qty_col]) if row[matl_qty_col] is not None else ''
...
def complete_kitting():
    """Complete a kitting - save to KITTING_DB and update MAIN_DB remaining qty"""
    try:
        data = request.json
        job_order = data.get('job_order', '')
        model_code = data.get('model_code', '')
        ...
        # Save to KITTING_DB
        result = db_manager.save_to_kitting_db(kitting_no, job_order, model_code, materials)
        ...
        if result:
            ...
            for material in materials:
                ...
                if qty_unit > 0:
                    # Deduct qty_unit from remaining_qty (QTY/Kit = QTY/Unit)
                    db_manager.update_main_db_after_kitting(job_order, row_no, qty_unit)
            
            return jsonify({
                "success": True, 
                "kitting_no": kitting_no,
                "message": f"Kitting #{kitting_no} completed successfully"
            })
        else:
