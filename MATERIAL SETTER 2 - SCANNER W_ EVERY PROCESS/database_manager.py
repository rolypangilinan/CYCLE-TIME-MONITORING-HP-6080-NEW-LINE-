...
class DatabaseManager:
    ...
    def save_to_main_db(self, job_order, model_code, materials):
        ...
        try:
            ...
            for material in materials:
                ...
                qty_kitting = int(material.get('qty_kitting', 0) or 0)
    ...
    def save_to_kitting_db(self, kitting_no, job_order, model_code, materials):
        ...
        try:
            ...
            (kitting_no, job_order, model_code, row_no, material_description, qty_unit, 
             material_code_scanned, lot_number, quantity_kitting, date_today)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """
            ...
            for material in materials:
                record = (
                    ...
                    material.get('scan_material', ''),
                    material.get('lot_no', ''),
                    int(material.get('qty_kitting', 0) or 0),
                    today
                )
                cursor.execute(insert_query, record)
            
            connection.commit()
            print(f"Saved kitting #{kitting_no} with {len(materials)} records to KITTING_DB")
            return True
    ...
    def update_main_db_after_kitting(self, job_order, row_no, qty_used):
        ...
        try:
            ...
            # Update qty_kitting (add to total used) and remaining_qty (subtract)
            update_query = """
            UPDATE MAIN_DB 
            SET qty_kitting = qty_kitting + %s,
                remaining_qty = remaining_qty - %s
            WHERE job_order = %s AND row_no = %s
            """
            cursor.execute(update_query, (qty_used, qty_used, job_order, row_no))
            connection.commit()
            print(f"Updated MAIN_DB row {row_no}: qty_kitting +{qty_used}, remaining_qty -{qty_used}")
            return True
